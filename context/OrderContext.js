...
export const OrderProvider = ({ children }) => {
  ...
  const fetchOrders = async () => { try { const res = await api.get('/orders'); setOrders(res.data || []); } catch(e) { console.log(e); } };
  const placeOrder = async (orderData) => { const res = await api.post('/orders', orderData); setCurrentOrder(res.data); return res.data; };
